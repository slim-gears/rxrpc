...
public class ServiceModules {
    ...
    static class ServiceSetModule<S> extends AbstractModule {
        ...
        protected void configure() {
            ...
            Collection<Class<? extends S>> serviceClasses = readServices(serviceClass);
    ...
    public static <S> Collection<Class<? extends S>> readServices(Class<S> serviceClass) {
        String resourcePath = String.format("/META-INF/services/%s", serviceClass.getName());
        try (InputStream stream = ServiceModules.class.getResourceAsStream(resourcePath)) {
            if (stream != null) {
                ...
                LOG.warn("Service configuration for {} not found in {}", serviceClass.getName(), resourcePath);
            }
        } catch (IOException e) {
            LOG.error("Cannot load service configuration: {}", e);
        }
    ...
    private static String nameFromAnnotation(Class cls) {
        return Optional
                ...
                .map(Named::value)
                .orElseGet(() -> Optional
                        .ofNullable((com.google.inject.name.Named)cls.getAnnotation(com.google.inject.name.Named.class))
